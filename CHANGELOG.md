...
- Accept CPython's native Windows `pyvenv.cfg` command form, preserve full 128-bit Windows file IDs while accepting Python 3.11's legacy identity pair during deletion, restore newly created hooks with Windows read-only semantics, and clean failed runtime releases through the portable Windows tree remover.
